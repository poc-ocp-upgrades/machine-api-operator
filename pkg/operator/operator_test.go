...
func TestOperatorSync_NoOp(t *testing.T) {
	...
	cases := []struct {
		...
		expectedNoop	bool
	}{{platform: configv1.AWSPlatformType, expectedNoop: false}, {platform: configv1.LibvirtPlatformType, expectedNoop: false}, {platform: configv1.OpenStackPlatformType, expectedNoop: false}, {platform: configv1.AzurePlatformType, expectedNoop: false}, {platform: bareMetalPlatform, expectedNoop: false}, {platform: kubemarkPlatform, expectedNoop: false}, {platform: configv1.VSpherePlatformType, expectedNoop: true}, {platform: configv1.NonePlatformType, expectedNoop: true}, {platform: "bad-platform", expectedNoop: true}}
	tempDir, err := ioutil.TempDir("", "TestOperatorSync")
	if err != nil {
		t.Fatalf("could not create the temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)
	images := Images{MachineAPIOperator: "test-mao"}
	...
	for _, tc := range cases {
		t.Run(string(tc.platform), func(t *testing.T) {
			infra := &configv1.Infrastructure{ObjectMeta: metav1.ObjectMeta{Name: "cluster"}, Status: configv1.InfrastructureStatus{Platform: tc.platform}}
			...
			optr := Operator{eventRecorder: record.NewFakeRecorder(5), osClient: fakeconfigclientset.NewSimpleClientset(infra), imagesFile: imagesFilePath, featureGateLister: featureGateInformer.Lister()}
			...
			expectedConditions := map[configv1.ClusterStatusConditionType]configv1.ConditionStatus{configv1.OperatorAvailable: configv1.ConditionTrue, configv1.OperatorProgressing: configv1.ConditionFalse, configv1.OperatorDegraded: configv1.ConditionTrue}
			if tc.expectedNoop {
...
func initOperator(featureGate *v1.FeatureGate, kubeclientSet *fakekube.Clientset) (*Operator, error) {
	...
	op := &Operator{kubeClient: kubeclientSet, featureGateLister: featureGateInformer.Lister(), deployLister: deployInformer.Lister(), ownedManifestsDir: "../../owned-manifests"}
...
func TestOperatorSyncClusterAPIControllerHealthCheckController(t *testing.T) {
	...
	}{{featureGate: &v1.FeatureGate{ObjectMeta: metav1.ObjectMeta{Name: MachineAPIFeatureGateName}, Spec: v1.FeatureGateSpec{FeatureSet: configv1.Default}}, expectedMachineHealthCheckController: false}, {featureGate: &v1.FeatureGate{}, expectedMachineHealthCheckController: false}, {featureGate: &v1.FeatureGate{ObjectMeta: metav1.ObjectMeta{Name: MachineAPIFeatureGateName}, Spec: v1.FeatureGateSpec{FeatureSet: configv1.TechPreviewNoUpgrade}}, expectedMachineHealthCheckController: true}}
	oc := OperatorConfig{TargetNamespace: targetNamespace, Controllers: Controllers{Provider: "controllers-provider", NodeLink: "controllers-nodelink", MachineHealthCheck: "controllers-machinehealthcheck"}}
