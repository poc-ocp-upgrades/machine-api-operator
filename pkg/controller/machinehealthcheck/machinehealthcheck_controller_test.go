...
func node(name string, ready bool) *v1.Node {
	...
	return &v1.Node{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: metav1.NamespaceNone, Annotations: map[string]string{"machine": fmt.Sprintf("%s/%s", namespace, "fakeMachine")}, Labels: map[string]string{}}, TypeMeta: metav1.TypeMeta{Kind: "Node"}, Status: corev1.NodeStatus{Conditions: []corev1.NodeCondition{{Type: corev1.NodeReady, Status: nodeReadyStatus, LastTransitionTime: knownDate}}}}
}
...
func TestGetNodeCondition(t *testing.T) {
	...
	}{{node: node("hasCondition", true), condition: &corev1.NodeCondition{Type: corev1.NodeReady, Status: corev1.ConditionTrue}, expected: &corev1.NodeCondition{Type: corev1.NodeReady, Status: corev1.ConditionTrue, LastTransitionTime: knownDate}}, {node: node("doesNotHaveCondition", true), condition: &corev1.NodeCondition{Type: corev1.NodeOutOfDisk, Status: corev1.ConditionTrue}, expected: nil}}
	for _, tc := range testsCases {
...
func TestReconcile(t *testing.T) {
	...
	fakeMachine.Status = mapiv1alpha1.MachineStatus{NodeRef: &corev1.ObjectReference{Namespace: "", Name: "healthy"}}
	testsCases := []struct {
		node		*v1.Node
		expected	expectedReconcile
	}{{node: nodeHealthy, expected: expectedReconcile{result: reconcile.Result{}, error: false}}, {node: nodeUnhealthy, expected: expectedReconcile{result: reconcile.Result{}, error: false}}, {node: nodeWithNoMachineAnnotation, expected: expectedReconcile{result: reconcile.Result{}, error: false}}, {node: nodeAnnotatedWithNoExistentMachine, expected: expectedReconcile{result: reconcile.Result{}, error: false}}}
	machineHealthCheck := machineHealthCheck("machineHealthCheck")
	allMachineHealthChecks := &healthcheckingv1alpha1.MachineHealthCheckList{Items: []healthcheckingv1alpha1.MachineHealthCheck{*machineHealthCheck}}
	r := newFakeReconciler(nodeHealthy, nodeUnhealthy, fakeMachine, allMachineHealthChecks)
	for _, tc := range testsCases {
		request := reconcile.Request{NamespacedName: types.NamespacedName{Namespace: "", Name: tc.node.Name}}
...
func TestHasMachineSetOwner(t *testing.T) {
	...
	testsCases := []struct {
		machine		*mapiv1alpha1.Machine
		expected	bool
	}{{machine: machineWithNoMachineSet, expected: false}, {machine: machineWithMachineSet, expected: true}}
	for _, tc := range testsCases {
...
func TestUnhealthyForTooLong(t *testing.T) {
	...
	testsCases := []struct {
		node		*corev1.Node
		expected	bool
	}{{node: nodeUnhealthyForTooLong, expected: true}, {node: nodeRecentlyUnhealthy, expected: false}}
	for _, tc := range testsCases {
...
func testRemediation(t *testing.T, remediationWaitTime time.Duration, initObjects ...runtime.Object) {
	...
	nodeHealthy.Annotations = map[string]string{"machine": fmt.Sprintf("%s/%s", namespace, "machineWithNodehealthy")}
	nodeRecentlyUnhealthy := node("nodeRecentlyUnhealthy", false)
	nodeRecentlyUnhealthy.Status.Conditions[0].LastTransitionTime = metav1.Time{Time: time.Now()}
	nodeRecentlyUnhealthy.Annotations = map[string]string{"machine": fmt.Sprintf("%s/%s", namespace, "machineWithNodeRecentlyUnhealthy")}
	machineWithNodeRecentlyUnhealthy := machine("machineWithNodeRecentlyUnhealthy")
	machineWithNodeRecentlyUnhealthy.Status = mapiv1alpha1.MachineStatus{NodeRef: &corev1.ObjectReference{Namespace: "", Name: "nodeRecentlyUnhealthy"}}
	machineWithNodeHealthy := machine("machineWithNodehealthy")
	machineWithNodeHealthy.Status = mapiv1alpha1.MachineStatus{NodeRef: &corev1.ObjectReference{Namespace: "", Name: "nodeHealthy"}}
	...
	testsCases := []struct {
		machine		*mapiv1alpha1.Machine
		expected	expectedReconcile
	}{{machine: machineWithNodeHealthy, expected: expectedReconcile{result: reconcile.Result{}, error: false}}, {machine: machineWithNodeRecentlyUnhealthy, expected: expectedReconcile{result: reconcile.Result{Requeue: true, RequeueAfter: remediationWaitTime}, error: false}}, {machine: machineWithNoOwnerController, expected: expectedReconcile{result: reconcile.Result{}, error: false}}, {machine: machineWithNoNodeRef, expected: expectedReconcile{result: reconcile.Result{}, error: true}}}
...
func TestIsMaster(t *testing.T) {
	...
	masterMachine.Status = mapiv1alpha1.MachineStatus{NodeRef: &corev1.ObjectReference{Namespace: "", Name: "master"}}
	masterNode := node("master", true)
	masterNode.Annotations = map[string]string{"machine": fmt.Sprintf("%s/%s", namespace, "master")}
	...
	workerMachine.Status = mapiv1alpha1.MachineStatus{NodeRef: &corev1.ObjectReference{Namespace: "", Name: "worker"}}
	workerNode := node("worker", true)
	workerNode.Annotations = map[string]string{"machine": fmt.Sprintf("%s/%s", namespace, "worker")}
	workerNode.Labels["node-role.kubernetes.io/worker"] = ""
	testCases := []struct {
		machine		*mapiv1alpha1.Machine
		expected	bool
	}{{machine: masterMachine, expected: true}, {machine: workerMachine, expected: false}}
